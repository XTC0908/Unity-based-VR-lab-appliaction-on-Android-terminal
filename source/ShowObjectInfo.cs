...
//添加组件菜单
//[AddComponentMenu("ShowObjectInfo")]
...
public class ShowObjectInfo : MonoBehaviour
{
    ...
    //  说明：变量声明部分
    //------------------------------------------------------------------------------------------------------------------
    public string text="键入你自己的内容";//文本内容
    public Camera TargetCamera;//面对的摄像机
    public bool ShowInfoInGamePlay = true;//是否在游戏运行时显示此信息框的标识符
    public bool ShowInfoInSceneEditor = true;//是否在场景编辑时显示此信息框的标识符
    ...
    private static GUIStyle Style
    {
        get
        {
            if (style == null)
            {
                ...
                //设置GUI的文本颜色
                style.normal.textColor = new Color(1.0f, 5.0f, 0.9f);
                //设置GUI的文本字体大小
                style.fontSize = 26;
            }
            return style;//采取上述style
        }
